		...
		<div class="padding15-25">
			<div class="single-content print-only">
				...
				<div class="single-topicos">
					...
					<div class="row single-topicos-open">
				...
				<?php
            		if (!empty($notes)) {
						...
						echo '</h2>';
						echo '</div>';
						echo '<div class="row single-topicos-open">';
			...
			<aside class="single-sidebar">
				...
				<div class="row single-topics-showall">
					<div class="i-show"></div>
					<?php _e( 'Expandir todos os tópicos','Ripsa'); ?>
				</div>
				<div class="row single-topics-hideall">
					<div class="i-hide"></div>
					<?php _e( 'Fechar todos os tópicos', 'Ripsa'); ?>
				</div>
				<div class="row bg-blue margintop10">
					<div class="padding7">
						<h3 class="row single-h3"><?php _e( 'Serviços', 'Ripsa'); ?></h3>
						<a href="<?php $key="bases_de_dados"; echo get_post_meta($post->ID,$key,true);?>" target="_blank" class="row single-servicos">
							<i class="i-basedados"></i><span class="single-servicos-text"><?php _e( 'Base de Dados', 'Ripsa'); ?></span>
						</a>
						<a href="<?php $key="literatura_scielo"; echo get_post_meta($post->ID,$key,true);?>" target="_blank" class="row single-servicos">
							<i class="i-scielo"></i><span class="single-servicos-text"><?php _e( 'Literatura Científica em SCIELO', 'Ripsa'); ?></span>
						</a>
						<a href="<?php $key="literatura_lilacs"; echo get_post_meta($post->ID,$key,true);?>" target="_blank" class="row single-servicos">
						...
						<?php 
		 				...
		 				?>
						<span id="impressao" class="row single-servicos">
						...
						<?php if (!empty($comment_ficha)) { ?>
							<a href="<?php the_field('comentarios_indicadores');?>" target="_blank" class="row single-servicos">
